...
class ESPLoader {
  ...
  command = async ({
    ...
    if (wait_response) {
      ...
      for (let i = 0; i < 100; i++) {
        var p = await this.transport.read({
          timeout: timeout
        });
  ...
  _connect_attempt = async ({
    ...
    while (1) {
      try {
        const res = await this.transport.read({
          timeout: 1000
        });
        i += res.length;
  ...
  connect = async ({
    ...
    for (i = 0; i < attempts; i++) {
      resp = await this._connect_attempt({
        mode: mode,
        esp32r0_delay: false
      });
      if (resp === "success") {
    ...
    if (!detecting) {
      var chip_magic_value = await this.read_reg({
        addr: 0x40001000
      }) >>> 0;
  ...
  run_spiflash_command = async (spiflash_command, data, read_bits) => {
    ...
    var old_spi_usr = await this.read_reg({
      addr: SPI_USR_REG
    });
    var old_spi_usr2 = await this.read_reg({
      addr: SPI_USR2_REG
    });
  ...
  run_stub = async () => {
    ...
    var chardata = decoded.split('').map(function(x) {
  ...
  write_flash = async ({
    ...
    console.log("EspLoader program", fileArray);
    if (flash_size !== 'keep') {
